class Player {
    ...
    static initialize() {
        ...
        document.addEventListener('touchend', (e) => { this.setKeyStatus("reset"); })
        ...
        const gesture = (xs, ys, xe, ye) => {
            ...
            if (Math.abs(horizonDirection) < Math.abs(verticalDirection)) {
                ...
                if (verticalDirection < 0) {
                    ...
                    setKeyStatus("down"); // down
                }
            } else {
                // 横方向
                if (horizonDirection < 0) {
                    setKeyStatus("left"); // left
                } else if (0 <= horizonDirection) {
                    setKeyStatus("right"); // right
                }
            }
        }
        */
    ...
    static playing(frame) {
        ...
        if (this.keyStatus.right || this.keyStatus.left) {
            // 左右の確認をする
            const rlx = (this.keyStatus.right) ? 1 : -1;
            ...
            if (y < 0 || x + rlx < 0 || x + rlx >= Config.stageCols || Stage.board[y][x + rlx]) {
                if (y >= 0) {
                    canMove = false;
                }
            }
            if (my < 0 || mx + rlx < 0 || mx + rlx >= Config.stageCols || Stage.board[my][mx + rlx]) {
                if (my >= 0) {
                    canMove = false;
                }
            }
            // 接地していない場合は、さらに１個下のブロックの左右も確認する
            if (this.groundFrame === 0) {
                if (y + 1 < 0 || x + rlx < 0 || x + rlx >= Config.stageCols || Stage.board[y + 1][x + rlx]) {
                    if (y + 1 >= 0) {
                        canMove = false;
                    }
                }
                if (my + 1 < 0 || mx + rlx < 0 || mx + rlx >= Config.stageCols || Stage.board[my + 1][mx + rlx]) {
                    if (my + 1 >= 0) {
                        canMove = false;
                    }
                }
            ...
            if (canMove) {
                ...
                this.moveDestination = (x + rlx) * Config.puyoImgWidth;
                this.puyoStatus.x += rlx;
                return 'moving';
            }
